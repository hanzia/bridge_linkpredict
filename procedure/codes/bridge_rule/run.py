...
from bridge_rule import Bridge_rules
...
def parse_args(args=None):
    ...
    parser.add_argument('--output', action='store_true', help = 'Output the entities score')
    ...
    #input storefile name with .json
    #parser.add_argument('--storefile', default=None, type = str)
...
def main(args):
    ...
    #laod all dataset
    train_triples = read_triple(os.path.join(args.data_path, 'train.txt'), entity2id)
    logging.info('#train: %d' % len(train_triples))
    valid_triples = read_triple(os.path.join(args.data_path, 'valid.txt'), entity2id)
    logging.info('#valid: %d' % len(valid_triples))
    test_triples = read_triple(os.path.join(args.data_path, 'test.txt'), entity2id)
    logging.info('#test: %d' % len(test_triples))
    addtrain_triples = read_triple(os.path.join(args.data_path, 'test.txt'), entity2id)
    ...
    if args.do_train:
        ...
        train_dataloader_head = DataLoader(
            TrainDataset(train_triples, nentity, args.negative_sample_size, 'head-batch'),
        ...
        train_dataloader_tail = DataLoader(
            TrainDataset(train_triples, nentity, args.negative_sample_size, 'tail-batch'),
            batch_size=args.batch_size,
            shuffle=True, 
            num_workers=max(1, args.cpu_num//2),
            collate_fn=TrainDataset.collate_fn
        ...
        optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, bdg_model.parameters()), 
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            log = bdg_model.train_step(bdg_model, optimizer, train_iterator, args)
            ...
            if args.do_valid and step % args.valid_steps == 0:
                ...
                metrics = bdg_model.test_step(bdg_model, valid_triples, all_true_triples, args)
    ...
    if args.do_test:
        ...
        metrics = bdg_model.test_step(bdg_model, test_triples, all_true_triples, args)
        log_metrics('Test', step, metrics)

    if args.evaluate_train:
        logging.info('Evaluating on Training Dataset...')
        metrics = bdg_model.test_step(bdg_model, train_triples, all_true_triples, args)
        log_metrics('Test', step, metrics)

    if args.output:
        logging.info('Output the entities rank...')
        storefile = 'score.txt'
        bdg_model.store_score(bdg_model, test_triples, all_true_triples, storefile, args)
